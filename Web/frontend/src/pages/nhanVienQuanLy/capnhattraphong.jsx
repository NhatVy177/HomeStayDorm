...
export default function CapNhatTraPhong() {
  const [dsHoanTat, setDsHoanTat] = useState([]);
  ...
  const submitXacNhan = async () => {
    if (chiTiet?.hasHopDong && !xacNhanRoi) {
    ...
    try {
      const payload = {
        ...
        danhSachBanGiao: chiTiet?.hasHopDong ? dsTaiSan.map(ts => ({
          ...ts,
          soLuongThuHoi: ts.soLuongThuHoi === '' ? 0 : Number(ts.soLuongThuHoi)
        })) : []
      };

      await capNhatTraPhongApi.xacNhanHoanTat(payload);
      loadDanhSach();
      setIsSubmitted(true);
    } catch (err) {
      console.error(err);
      setErrorMsg(err.response?.data?.message || 'Lỗi khi cập nhật hoàn tất trả phòng.');
    }
  };
          ...
          <div className="ktp-modal" style={{ maxWidth: '950px', width: '90%', padding: '0', backgroundColor: '#ffffff', borderRadius: '12px', display: 'flex', flexDirection: 'column', maxHeight: '90vh' }} onClick={(e) => e.stopPropagation()}>
            <div className="ktp-modal-header" style={{ padding: '16px 24px', backgroundColor: '#3b8280', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderRadius: '12px 12px 0 0', flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <h3 style={{ fontSize: '20px', margin: 0, color: '#ffffff', fontWeight: '700' }}>Cập nhật hoàn tất hồ sơ</h3>
              </div>
            ...
            <div className="ktp-modal-body" style={{ padding: '20px 24px', overflowY: 'auto', backgroundColor: '#f8f9fa', flex: 1 }}>
              {loadingDetail ? (
                ...
                <>
                  ...
                  {chiTiet.hasHopDong ? (
                    <div style={{ backgroundColor: '#ffffff', border: '1px solid #e1e3e4', borderRadius: '8px', padding: '20px', marginBottom: '20px' }}>
                      <h4 style={{ fontSize: '16px', fontWeight: '700', color: '#191c1d', margin: '0 0 16px 0' }}>Ghi nhận bàn giao ra</h4>
                      {dsTaiSan.length > 0 ? (
                    ...
                    <div style={{ backgroundColor: '#ffffff', border: '1px solid #e1e3e4', borderRadius: '8px', padding: '16px', marginBottom: '20px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', color: '#137333' }}>
                        <Icon name="check_circle" />
                        <span style={{ fontWeight: 500 }}>Hồ sơ đặt cọc (chưa có hợp đồng) không phát sinh thủ tục bàn giao tài sản. Có thể hoàn tất ngay.</span>
                      </div>
                    </div>
                  )}
