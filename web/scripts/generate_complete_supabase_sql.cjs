...
function convertBatch(sql) {
  ...
  // Error handling
  s = s.replace(/THROW\s+\d+\s*,\s*([^,;]+)\s*,\s*\d+;?/gi, 'RAISE EXCEPTION %;', '$1');
  s = s.replace(/RAISERROR\s*\(\s*([^,]+)\s*,\s*\d+\s*,\s*\d+\s*\);?/gi, 'RAISE EXCEPTION %;', '$1');
...
const files = [
  'Database/Trigger/trigger.sql',
  'Database/SP_Chung/auth.sql',
  'Database/SP_Chung/cap-nhat-hoa-don-qua-han.sql',
  'Database/SP_DKyThue/dang-ky-thue.sql',
  'Database/SP_DKyThue/khach-moi.sql',
  'Database/SP_DKyThue/sp_tudongxuly.sql',
  'Database/SP_DatCoc/dat-coc-chung.sql',
  'Database/SP_DatCoc/dat-coc-gui-yeu-cau.sql',
  'Database/SP_DatCoc/dat-coc-lap-phieu.sql',
  'Database/SP_DatCoc/dat-coc-xac-nhan-kha-nang.sql',
  'Database/SP_DatCoc/dat-coc-thanh-toan.sql',
  'Database/SP_DatCoc/dat-coc-xac-nhan-thanh-toan.sql',
  'Database/SP_DatCoc/dat-coc-chot-phieu.sql',
  'Database/SP_NhanPhong/sp_cu_tru_nhan_phong.sql',
  'Database/SP_NhanPhong/sp_lap_hop_dong_thue.sql',
  'Database/SP_NhanPhong/sp_lap_bien_ban_ban_giao.sql',
  'Database/SP_NhanPhong/sp_ghi_nhan_khoan_thu_nhan_phong.sql',
  'Database/SP_NhanPhong/sp_hopdong_chitiet.sql',
  'Database/SP_NhanPhong/sp_gethopdong.sql',
  'Database/SP_TraPhong/00_All_SP_TraPhong_AIO.sql',
  'Database/SP_Admin/admin.sql'
];

let fullSql = `-- ============================================================================
-- SUPABASE POSTGRESQL MIGRATION: ALL TRIGGERS & PROCEDURES
-- Hệ thống Quản lý Homestay / Dormitory
-- ============================================================================
...
-- ----------------------------------------------------------------------------
-- PHẦN 1: DATABASE TRIGGERS (9 TRIGGERS ĐÃ CONVERT SANG POSTGRESQL)
-- ----------------------------------------------------------------------------
...
console.log('Finished writing base triggers to:', targetPath);
